...
class StopwatchApp:
    def __init__(self, master):
        ...
        self.add_time_entry = tk.Entry(master, font=("Arial", 14))
        self.add_time_entry.pack(pady=10)

        self.add_time_button = tk.Button(master, text="Add Time", command=self.add_time_to_stopwatch, font=("Arial", 16), bg="white", fg="black", width=10)
        self.add_time_button.pack(pady=10)
    ...
    def add_time_to_stopwatch(self):
        time_to_add_str = self.add_time_entry.get()
        try:
            time_to_add = float(time_to_add_str)  # Convert the input to float
            if time_to_add > 0:
                if self.start_time is not None:
                    self.start_time -= timedelta(minutes=time_to_add)  # Subtract time from start time
                    self.label.config(text="Time added to stopwatch.")
                else:
                    self.label.config(text="Start the stopwatch first.")
            else:
                self.label.config(text="Please enter a positive number.")
        except ValueError:
            self.label.config(text="Invalid input. Please enter a number.")

        self.add_time_entry.delete(0, tk.END)
def main():
    root = tk.Tk()
